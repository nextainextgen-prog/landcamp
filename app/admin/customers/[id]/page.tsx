...
export default async function CustomerDetailPage({ params }: Ctx) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title={name}
        description="โปรไฟล์ลูกค้าและประวัติการจอง"
        actions={
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* CRM placeholders — backend wired later (see memory: admin-backend-todo) */}
          <Panel title="โน้ต / บันทึก">
            <ComingSoon>ระบบบันทึกโน้ตลูกค้า กำลังพัฒนา</ComingSoon>
          </Panel>
          <Panel title="ประวัติการติดต่อ">
            <ComingSoon>บันทึกการโทร/อีเมล/แชต กำลังพัฒนา</ComingSoon>
          </Panel>
        </div>
      </div>
    </div>
...
function ComingSoon({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-dashed border-[color:var(--color-forest-deep)]/20 bg-[color:var(--color-bone-soft)]/30 px-4 py-6 text-center text-xs text-[color:var(--color-ink)]/45">
      {children}
    </div>
  );
}
